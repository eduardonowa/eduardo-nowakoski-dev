...
describe('CodeSnippets', () => {
  ...
  it('should regenerate snippets on interval', async () => {
    ...
    await waitFor(() => {
      const snippets = screen.queryAllByText(/const developer|function buildPortfolio|@Component/i)
      expect(snippets.length).toBeGreaterThan(0)
    })
  ...
  // Cenários com `window` indefinido não são simulados aqui, pois o React DOM
  // em ambiente de testes (jsdom) ainda depende da presença de `window`.
  ...
  it('should not generate snippets when isVisible is false', () => {
    ...
    // Fast-forward less than initial delay (500ms)
    act(() => {
